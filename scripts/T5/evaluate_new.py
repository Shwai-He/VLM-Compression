...
if instruct == "true":
    if cc3m == "true":
        ...
        pretrain = "continue_stage2_t5_instruct"
else:
    if cc3m == "true":
...
job_id = f"{method}/{pretrain}-{pruner}_{kl_weight}_{t5_ratio}_{vit_ratio}" if prune_n == 0 else f"{method}/{pretrain}-{pruner}_{kl_weight}_{prune_n}:{prune_m}"
job_id = job_id + "_" + tune_opt + "_" + remain_grads + "_" + str(int(max_train_samples))

for task in ["okvqa_zeroshot_flant5xl_eval", "gqa_zeroshot_flant5xl_eval", "nocaps_flant5xl_eval",
             "vqav2_zeroshot_flant5xl_eval", "ret_flickr_eval"]:
    if instruct == "true":
