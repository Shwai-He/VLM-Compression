...
def _prepare_lora_config(peft_config, model_config, model_type=None):
    Model_Type = model_type if model_type is not None else model_config["model_type"]
    if peft_config.target_modules is None:
        if Model_Type not in TRANSFORMERS_MODELS_TO_LORA_TARGET_MODULES_MAPPING:
            raise ValueError("Please specify `target_modules` in `peft_config`")
        peft_config.target_modules = TRANSFORMERS_MODELS_TO_LORA_TARGET_MODULES_MAPPING[Model_Type]
...
def _prepare_bottleneck_config(peft_config, model_config, model_type=None):
    Model_Type = model_type if model_type is not None else model_config["model_type"]
    if peft_config.target_modules is None:
        if peft_config.use_parallel_adapter:
            if Model_Type not in TRANSFORMERS_MODELS_TO_PARALLEL_TARGET_MODULES_MAPPING:
                raise ValueError("Please specify `target_modules` in `peft_config`")
            peft_config.target_modules = TRANSFORMERS_MODELS_TO_PARALLEL_TARGET_MODULES_MAPPING[Model_Type]
        elif peft_config.use_adapterp:
            if Model_Type not in TRANSFORMERS_MODELS_TO_ADAPTERP_TARGET_MODULES_MAPPING:
                raise ValueError("Please specify `target_modules` in `peft_config`")
            peft_config.target_modules = TRANSFORMERS_MODELS_TO_ADAPTERP_TARGET_MODULES_MAPPING[Model_Type]
        else:
            if Model_Type not in TRANSFORMERS_MODELS_TO_BOTTLENECK_TARGET_MODULES_MAPPING:
                raise ValueError("Please specify `target_modules` in `peft_config`")
            peft_config.target_modules = TRANSFORMERS_MODELS_TO_BOTTLENECK_TARGET_MODULES_MAPPING[Model_Type]
...
def get_peft_model(model, peft_config):
    ...
    if peft_config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys():
        if peft_config.peft_type == "LORA":
            peft_config = _prepare_lora_config(peft_config, model_config, model_type)
            return PeftModel(model, peft_config)
        elif peft_config.peft_type == "BOTTLENECK":
            peft_config = _prepare_bottleneck_config(peft_config, model_config, model_type)
            return PeftModel(model, peft_config)
