...
class RunnerBase:
    ...
    def model(self):
        ...
        if self._model.device != self.device and self._model.device.type != self._device.type:
            # print(f"self._model.device: {self._model.device}")
            # print(f"Move model to {self.device}")
            self._model = self._model.to(self.device)
        # print(f"self._wrapped_model: {self._wrapped_model}")
        # print(f"self.use_distributed: {self.use_distributed}")

        # if self.use_distributed and self.adapter_name != "lora":
    ...
    def train(self, prune_retrain=False):
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            # print(f"self.valid_splits: {self.valid_splits}")
            # evaluation phase
            ...
            if self.evaluate_only or prune_retrain:
                break
    ...
    def compute_sim_matrix(self, split_name, cuda_enabled=True, **kwargs):
        data_loader = self.dataloaders.get(split_name, None)
        ...
        start_time = time.time()
        print(f"data_loader.dataset: {data_loader.dataset}")
        print(f"data_loader.dataset: {data_loader.dataset.keys()}")
        print_freq = 10
        ...
        for i in range(0, num_text, text_bs):
            ...
            text_feat = self.model.forward_text(text_input)
            text_embed = F.normalize(self.model.text_proj(text_feat))
        ...
        for samples in data_loader:
            ...
            image_feat, vit_feat = self.model.forward_image(image)
            image_embed = self.model.vision_proj(image_feat)
        ...
        for i, sims in enumerate(
            ...
            image_inputs = vit_feats[start + i].repeat(k_test, 1, 1).to(self.model.device)
