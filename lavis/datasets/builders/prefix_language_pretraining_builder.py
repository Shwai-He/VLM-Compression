...
from lavis.datasets.datasets.prefix_language_pretraining import PrefixLanguagePretrainingDataset, InstructLanguagePretrainingDataset
...
class PrefixConceptualCaption3MBuilder(BaseDatasetBuilder):
    ...
    eval_dataset_cls = PrefixLanguagePretrainingDataset
...
class PrefixVGCaptionBuilder(BaseDatasetBuilder):
    ...
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/vg/prefix_caption.yaml"
        }

@registry.register_builder("prefix_coco_caption")
class PrefixCOCOCaptionBuilder(BaseDatasetBuilder):
    train_dataset_cls = PrefixLanguagePretrainingDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/coco/prefix_cap.yaml",
        }
    
@registry.register_builder("instruct_vg_caption")
class InstructVGCaptionBuilder(BaseDatasetBuilder):
    train_dataset_cls = InstructLanguagePretrainingDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/vg/instruct_caption.yaml"
        }

@registry.register_builder("instruct_coco_caption")
class InstructCOCOCaptionBuilder(BaseDatasetBuilder):
    train_dataset_cls = InstructLanguagePretrainingDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/coco/instruct_cap.yaml",
        }

@registry.register_builder("instruct_cc3m_caption")
class InstructConceptualCaption3MBuilder(BaseDatasetBuilder):
    train_dataset_cls = InstructLanguagePretrainingDataset
    # eval_dataset_cls = InstructLanguagePretrainingDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/prefix_conceptual_caption/instruct_3m.yaml"
    }
