...
class Blip2OPT(Blip2Base):
    ...
    def __init__(
        ...
        if opt_model in local_paths and os.path.isdir(local_paths[opt_model]):
            self.opt_tokenizer = AutoTokenizer.from_pretrained(local_paths[opt_model], use_fast=False)
            self.opt_model = OPTForCausalLM.from_pretrained(
                local_paths[opt_model], torch_dtype=torch.float16
            )
        else:
