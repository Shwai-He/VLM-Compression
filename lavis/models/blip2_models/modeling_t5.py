...
class T5DenseActDense(nn.Module):
    ...
    def forward(self, hidden_states, dense=False):
        if not dense:
            ...
            hidden_states = self.wi(hidden_states, dense=dense)
            hidden_states = self.act(hidden_states, dense=dense)
            hidden_states = self.dropout(hidden_states, dense=dense)
            hidden_states = self.wo(hidden_states)
...
class T5DenseGatedActDense(nn.Module):
    ...
    def forward(self, hidden_states, dense=False):
        if not dense:
            ...
            hidden_gelu = self.act(self.wi_0(hidden_states, dense=dense))
            hidden_linear = self.wi_1(hidden_states, dense=dense)
            hidden_states = hidden_gelu * hidden_linear
            hidden_states = self.dropout(hidden_states)
            hidden_states = self.wo(hidden_states, dense=dense)
...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, dense=False):
        ...
        if not dense:
            ...
            forwarded_states = self.DenseReluDense(forwarded_states, dense)
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if key_value_states is None:
                ...
                if not dense:
                    ...
                    hidden_states = shape(proj_layer(hidden_states, dense=dense))
                ...
                if not dense:
                    ...
                    hidden_states = shape(proj_layer(key_value_states, dense=dense))
        ...
        if not dense:
            ...
            query_states = shape(
                self.q(hidden_states, dense=dense)
            )  # (batch_size, n_heads, seq_length, dim_per_head)
        ...
        if not dense:
            ...
            attn_output = self.o(attn_output, dense=dense)
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.layer[-1](hidden_states, dense=dense)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if  encoder_outputs.hidden_states is not None and decoder_outputs.hidden_states is not None:
