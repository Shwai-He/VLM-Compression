...
# from transformers.models.instructblip import InstructBlipForConditionalGeneration, InstructBlipProcessor
...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        if llm_model in local_paths and os.path.isdir(local_paths[llm_model]) and os.path.isdir(local_paths[llm_model]):
            self.llm_tokenizer = LlamaTokenizer.from_pretrained(local_paths[llm_model], use_fast=False, truncation_side="left")
            self.llm_model = LlamaForCausalLM.from_pretrained(
                local_paths[llm_model], torch_dtype=torch.float16
            )
        else:
    ...
    def generate(
        ...
        if "prompt" in samples.keys():
            ...
            if isinstance(self.prompt, str):
                prompt = self.prompt
            elif isinstance(self.prompt, omegaconf.listconfig.ListConfig):
