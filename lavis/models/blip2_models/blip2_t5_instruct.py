...
class Blip2T5Instruct(Blip2Base):
    ...
    def __init__(
        ...
        if t5_model in local_paths and os.path.isdir(local_paths[t5_model]):
            self.t5_tokenizer = T5TokenizerFast.from_pretrained(local_paths[t5_model], truncation_side='left')
            self.t5_output_tokenizer = T5TokenizerFast.from_pretrained(local_paths[t5_model], truncation_side='right')
