...
class Mlp(nn.Module):
    ...
    def forward(self, x, dense=False):
        if dense == True: # False is the default for lora models and vanilla models. 
            x = self.fc1(x, dense)
            x = self.act(x)
            # x = self.drop(x)
...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, dense=False):
        B, N, C = x.shape
        qkv_bias = None
    ...
    def forward(self, x, rel_pos_bias=None, dense=False):
        if dense == True:
            if self.gamma_1 is None:
                x = x + self.drop_path(self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, dense=dense))
                x = x + self.drop_path(self.mlp(self.norm2(x), dense))
            else:
                x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, dense=dense))
                x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x), dense))
        else:
            if self.gamma_1 is None:
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, dense=False):
        ...
        for blk in self.blocks:
            ...
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, rel_pos_bias, dense)
            else:
                x = blk(x, rel_pos_bias, dense)
    ...
    def forward(self, x, dense=False):
        x = self.forward_features(x, dense)
